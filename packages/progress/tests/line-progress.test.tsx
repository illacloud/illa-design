...
test("Progress renders with colors", () => {
  render(
    <div>
      <Progress data-testid="test-progress-red" type="line" color="red" trailColor="red" />
      <Progress data-testid="test-progress-gray" type="line" color="gray" trailColor="gray" />
      <Progress data-testid="test-progress-custom" type="line" color="#123123" trailColor="#321321" />
      <Progress data-testid="test-progress-red-step" type="line" steps={3} color="red" trailColor="red" />
      <Progress data-testid="test-progress-gray-step" type="line" steps={3} color="gray" trailColor="gray" />
      <Progress data-testid="test-progress-custom-step" type="line" steps={3} color="#123123" trailColor="#321321" />
    </div>,
  )
...
test("Progress renders with format text function", () => {
  render(<Progress percent={50} type="line" formatText={(percent) => {
    return `test${percent}`
  }
  } />)
...
test("Progress renders with custom width", () => {
  render(<Progress data-testid="test-progress" percent={50} width="100px" type="line" strokeWidth="10px" />)
  expect(screen.getByTestId("test-progress")).toMatchSnapshot()
})

test("Progress renders with steps", () => {
  render(<Progress data-testid="test-progress" steps={3} type="line" percent={40} />)
  expect(screen.getByTestId("test-progress")).toMatchSnapshot()
})