...
export const PageSizeSelector = forwardRef<HTMLSpanElement, PageSizeSelectorProps>((props, ref) => {
    const {
        onPageSizeSelected,
        sizeOptions,
        wholeDisable,
    ...
    return <span>
        <select css={applyPageSizeSelectorCss(wholeDisable, size)}
           ...
           {sizeOptions.map(((value) => <option key={value} value={value}>{value + `/${unitText}`}</option>))}
          </select>
    </span>
})