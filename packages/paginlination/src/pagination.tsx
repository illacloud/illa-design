...
import {applyDefaultItemCss, applyDefaultItemWithMarginCss, paginationContainer, totalTextCss} from "./style";
...
export const Pagination = forwardRef<HTMLDivElement, PaginationProps>((props, ref) => {
    ...
    if (typeof showTotal === "boolean" && showTotal){
        totalElement =  <span css={totalTextCss}>{totalText.replace("{0}", total.toString())}</span>
    }
    if (typeof showTotal === "function"){
        totalElement =  showTotal
    }

    return <div css={paginationContainer} placeholder={placeholder}>
        {totalElement}
        {simple ? <SimplePagination defaultSelectedPageIndex={curPage}
                                    pageSum={pageSum}
                                    size={size}
            ...
            : <span css={paginationContainer}><span
                ...
                }>{icons.prev}</span>{pageSum > 0 && <PageNumGroup total={pageSum}
                                                                   moreIcon={icons.more}
                ...
                <span css={applyDefaultItemCss(nextDisable, size)}
                      onClick={() => {
                          if (nextDisable) return
                          setCurPage(curPage + 1)
                      }}>{icons.next}</span></span>}

        {!simple && showJumper && <JumperInput
            wholeDisable={disabled}
            size={size}
            onEnterPress={pageNum => {
                if (pageNum < 1) {
                    setCurPage(0)
                } else if (pageNum > pageSum - 1) {
                    setCurPage(pageSum - 1)
                } else {
                    setCurPage(pageNum - 1)
                }
            }}/>}
        {!simple && sizeCanChange && <PageSizeSelector sizeOptions={sizeOptions}
                                                       wholeDisable={disabled}
                                                       size={size}
