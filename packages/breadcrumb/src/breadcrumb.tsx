...
export const Breadcrumb = forwardRef<HTMLDivElement, BreadcrumbProps>((props, ref) => {
  ...
  function Separator() {
    return (
      <>
        {
          separator ? separator : <span css={separatorCss}>/</span>
        }
      </>
  ...
  const routeItems = routes?.length && routes.map((item, idx: number) => {
    return (
      <>
        ...
        >
          {
            maxCount && routes.length > maxCount ?
              idx === 0 || idx > routes.length - maxCount ? <BreadcrumbItem /> : <span css={itemCss}>...</span>
              :
              <BreadcrumbItem />
          }
        </BreadcrumbContext.Provider>
        ...
        >
          {
            maxCount && childLiItem.length > maxCount ?
              idx === 0 || idx > childLiItem.length - maxCount ? <>{ele}</> : <span css={itemCss}>...</span>
              :
              <>{ele}</>
          }
        </BreadcrumbContext.Provider>
