...
export const RangePicker: FC<RangeDatePickerProps> = (props) => {
  ...
  function onHandleChange(newValue: Dayjs[] | undefined) {
    if (isDayjsArrayChange(mergedValue, newValue)) {
      const localValue = Array.isArray(newValue)
        ? newValue.map((v) =>
            getLocaleDayjsValue(toLocal(v, utcOffset, timezone), "en"),
          )
        : undefined
      onChange &&
        onChange(
  ...
  function onClickConfirmBtn() {
    onConfirmValue()
    const localePanelValue = panelValue.map((v) => getLocaleDayjsValue(v, "en"))
    onOk &&
      onOk(
  ...
  function onMouseEnterCell(date: Dayjs, disabled: boolean) {
    ...
    if (!disabled) {
      const placeHolderValue = showTime
        ...
        : date
      setHoverPlaceholderValue(placeHolderValue.locale("en").format(format))
    }
  }
  ...
  function changePageShowDates(type: "prev" | "next", unit: UnitType, num = 1) {
    ...
    if (type === "prev") {
      // @ts-ignore
      newPageShowDates[index] = mergedPageShowDate?.[index].subtract(num, unit)
    }
    if (type === "next") {
      // @ts-ignore
  ...
  const shouldShowFooter =
    (showTime && panelModes[0] === "date" && panelModes[1] === "date") || extra
